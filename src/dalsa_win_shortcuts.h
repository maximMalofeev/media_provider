...
template <typename T>
bool getFeatureValue(SapAcqDevice* AcqDevice, const std::string& featureName, T& value, std::string& error) {
...
template <typename T>
bool setFeatureValue(SapAcqDevice* AcqDevice, const std::string& featureName, const T& featureValue, std::string& error) {
	BOOL isAvailable = 0;
	error = "";
...
template<typename T>
bool getMinAndMaxValues(SapAcqDevice* AcqDevice, const std::string serverName, const std::string& featureName, T& min, T& max, std::string& error) {
	BOOL isAvailable = 0;
	error = "";
	
	SapFeature feature(serverName.c_str());
	if (!feature.Create()) {
		error.append("Unable to create feature object for ").append(featureName).append(", reason: ").append(feature.GetLastStatus());
		return false;
	}
	...
	if (AcqDevice->IsFeatureAvailable(featureName.c_str(), &isAvailable), isAvailable) {
		if (AcqDevice->GetFeatureInfo(featureName.c_str(), &feature)) {
			if (!feature.GetMin(&min)) {
				error.append("Unable to get min ").append(featureName).append(", reason: ").append(feature.GetLastStatus());
				return false;
			}
			if (!feature.GetMax(&max)) {
				error.append("Unable to get max ").append(featureName).append(", reason: ").append(feature.GetLastStatus());
				return false;
			}
			return true;
		}
		else {
			error.append("Unable to get ").append(featureName).append(" info, reason: ").append(feature.GetLastStatus());
			return false;
		}
	}
	else {
...
template<typename T>
bool setFeatureValueInAvaliableRange(SapAcqDevice* AcqDevice, const std::string serverName, const std::string& featureName, T& featureValue, std::string& error) {
	T minValue = T();
	T maxValue = T();
	if (!getMinAndMaxValues<T>(AcqDevice, serverName, featureName, minValue, maxValue, error)) {
		return false;
	}

	if (featureValue < minValue || featureValue > maxValue) {
		featureValue < minValue ? featureValue = minValue : featureValue = maxValue - 1;
	}
